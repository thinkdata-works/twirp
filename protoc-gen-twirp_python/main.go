...
func (g *generator) generateProtobufClient(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) {
	...
	g.P(`    def __init__(self, server_address, request_decorator=None):`)
	...
	g.P(`            request_decorator: A function to modify the http request being`)
	g.P(`		         sent. Takes in a urllib.request.Request object and returns the`)
	g.P(`		         same request object`)
	g.P(`        """`)
	...
	g.P(``)
	g.P(`        self.__request_decorator = request_decorator`)
	g.P()
	...
	g.P(`        )`)
	g.P(``)
	g.P(`        if self.__request_decorator:`)
	g.P(`            req = self.__request_decorator(req)`)
	g.P(``)
	g.P(`        try:`)
